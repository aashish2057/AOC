...
impl Presents {
    ...
    fn shortest_perimeter(&self) -> u32 {
        return (2 * self.perimeter1()).min(2 * self.perimeter2()).min(2 * self.perimeter3());
    }
...
fn part1(input: &Vec<String>) -> u32 {
    ...
    for s in input {
        ...
        let _p = Presents {
            length: vals.get(0).expect("Missing number").parse::<u32>().expect("Not valid number"),
            width: vals.get(1).expect("Missing number").parse::<u32>().expect("Not valid number"),
            height: vals.get(2).expect("Missing number").parse::<u32>().expect("Not valid number"),
        };
...
mod test {
    ...
    #[test]
    fn test_part1() {
        let v = vec![String::from("13x14x12"), String::from("15x14x18"), String::from("8x6x4")];
